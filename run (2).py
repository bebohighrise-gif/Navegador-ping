...
Anti-detección completa con curl_cffi
"""
...
# ===== CRITICAL: Solo usar curl_cffi para peticiones HTTP =====
try:
    from curl_cffi import requests
    CURL_CFFI_AVAILABLE = True
    print("✅ curl_cffi disponible - Anti-detección activada")
except ImportError:
    # Fallback a requests normal SOLO si curl_cffi falla
    import requests as _requests
    requests = _requests
    CURL_CFFI_AVAILABLE = False
    print("⚠️ WARNING: curl_cffi NO disponible - Usando requests (SERÁ BLOQUEADO)")
...
# Log curl_cffi availability AFTER logger is configured
...
def ping_service():
    ...
    try:
        ...
        # Usar curl_cffi con impersonate
        if CURL_CFFI_AVAILABLE:
...
def proxy():
    ...
    try:
        ...
        # Anti-detección: delay aleatorio (0.1-0.5s)
        time.sleep(random.uniform(0.1, 0.5))
        
        # CRÍTICO: Usar impersonate SIEMPRE (curl_cffi)
        if CURL_CFFI_AVAILABLE:
            logger.debug("🎭 Usando curl_cffi con impersonate='chrome120'")
            response = requests.get(
                url,
                headers=headers,
                timeout=Config.PROXY_TIMEOUT,
                allow_redirects=True,
                impersonate="chrome120"  # 🔥 TLS fingerprint de Chrome real
            )
        else:
            # Fallback (será bloqueado en muchos sitios)
            logger.warning("⚠️ Usando requests sin impersonate - Alto riesgo de bloqueo")
