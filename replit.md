...
- 2026-04-30: **Ban con motivo + notificación.** El admin ahora aplica baneos (temporales o permanentes) desde un modal que pide un motivo obligatorio (≥5 caracteres, presets clicables: Spam / Abuso / Contenido inapropiado / Violación TOS), permite elegir duración con presets (1h/24h/3d/7d/30d) y opcionalmente notifica al usuario por email (SendGrid). Backend: nuevos campos `ban_reason`, `ban_by`, `ban_at` en cada usuario; `is_banned()` ahora devuelve un mensaje que incluye motivo y admin que aplicó el baneo, así que la pantalla `/login` muestra a la víctima el motivo automáticamente. La sesión activa del usuario baneado se cierra a los 4s (próximo poll → 401 → redirect a /login). El listado admin muestra el motivo en una banda roja debajo de cada usuario baneado.
